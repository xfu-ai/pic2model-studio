...
def test_b01_11_export_replay_does_not_require_an_unconsumed_capability(tmp_path: Path):
    ...
    assert first.status_code == second.status_code == 200
    assert second.json() == first.json()
    assert (export_directory / "Demo-backup.pic2model").is_file()
...
def test_b01_11_diagnostics_export_writes_zip_inside_authorized_folder(tmp_path: Path):
    ...
    assert exported.json()["path"] == "Pic2Model-Studio-diagnostics.zip"
    assert (destination / "Pic2Model-Studio-diagnostics.zip").is_file()
